...
This chapter will guide users to quickly start using GridBeat to collect data simulating Modbus equipment and upload the data to MQTT Broker, taking Modbus TCP driver protocol as an example.

## Install GridBeat

GridBeat provides a variety of installation methods, and users can view the detailed installation methods in [Installation](../installation/installation.md). This example adopts containerized deployment mode, so as to start experiencing GridBeat as soon as possible.
...
::: tip
GridBeat and the simulator must be run on the same LAN.

Try to turn off the firewall in Windows, otherwise GridBeat may not connect to the simulator.
:::

## Quick Using GridBeat

Access the GridBeat gateway management console, open a web browser and enter the address and port number of the gateway running GridBeat ([http://x.x.x.x:7000](http://x.x.x.x:7000/)). Replace "x.x.x.x" with the actual IP address of the gateway where GridBeat is installed. 
...
In GridBeat, southbound refers to the connection between GridBeat and devices. This step illustrates how to create a southbound device.
...
### Connect the Device to GridBeat

#### Set Device Address

Configure the parameters required for GridBeat to establish a connection with the device.
...
In GridBeat, northbound refers to the connection between GridBeat and MQTT broker. . This step illustrates how to create a southbound device.
...
Configure the parameters required for GridBeat to establish a connection with the northbound application.
...
* **Upload topic**: Use the default escalation topic (/gridbeat/mqtt/upload).
...
After successful subscription, we can see that MQTT X can directly receive the data collected and reported by GridBeat.
...
* Add a new subscription, and the **Topic** should be consistent with the **Upload topic** in the applicatin configuration step, for example, `/gridbeat/mqtt/upload`.

:::tip
The default topic format for uploading topic is `/gridbeat/{node_name}/upload`, where `{node_name}` is the name of the created northbound application. Users can also customize the reporting theme.
:::
